...
type Props = { draggable?: boolean; inSidebar?: boolean; selected?: boolean; initialValue?: string; data: { type: string } };

const ClassNode = ({ draggable, inSidebar = false, selected = false, initialValue = "Sınıf" }: Props) => {
  ...
  // Varsayılan sınıf rengi (sarı) style.module.scss içinde .header'da tanımlı
  const containerClass = `${style.classNode} ${inSidebar ? style.preview : ''} nodes ${NodeType.SINIF}`;

  if (inSidebar) {
    return (
      <div className={containerClass} draggable={draggable}>
        <div className={style.header}><span className={style.title}>{initialValue}</span></div>
      </div>
    );
  }
  ...
  return (
    <>
      <NodeResizer isVisible={selected} color="#FBC02D" minWidth={140} />
