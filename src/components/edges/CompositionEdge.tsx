...
export default function CompositionEdge(props: EdgeProps) {
  const { id, sourceX, sourceY, targetX, targetY, style } = props;
  const [path] = getStraightPath({ sourceX, sourceY, targetX, targetY });
  const markerId = `uml-comp-${id}`;
  return (
    <>
      <defs>
        <marker id={markerId} markerWidth={26} markerHeight={26} refX={24} refY={13} orient="auto">
          <path d="M 2 13 L 13 2 L 24 13 L 13 24 Z" fill="#222" stroke="#222" strokeWidth="1.5" />
        </marker>
      </defs>
